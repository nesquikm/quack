...
// Reads stdin → parseHookPayload → dispatchHook("stop") → exit 0.
...
async function main(): Promise<void> {
  try {
    ...
    if (data === null) return;
    await dispatchHook({ kind: "stop", payload: data });
  } catch (err) {
    try {
      process.stderr.write(`[quack-hook plugin] stop error: ${String(err)}\n`);
    } catch {}
  }
}
