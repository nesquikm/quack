...
- plugin.json registration keys (Pass 2 round 2, advisory) — review suggested adding explicit `hooks` / `mcp-servers` / `commands` registration keys to `plugins/quack/.claude-plugin/plugin.json`. **Originally marked refuted by precedent**, then corrected during the AC.11 smoke prep: the dev-process-toolkit reference doesn't ship hooks at all, so its precedent only covered `commands/` (which IS directory-discovered) and didn't say anything about hooks. **Hooks require an explicit `plugins/quack/hooks/hooks.json` manifest** keyed by Claude Code event name (`SessionStart` / `Stop` / `PostToolUse`), with each entry pointing at the corresponding shell script via `${CLAUDE_PLUGIN_ROOT}/hooks/<name>.sh`. Without that manifest the wrapper scripts never fire (silent no-op) — exactly the failure mode the reviewer warned about. Fixed in a follow-up commit; `tests/plugin-files.test.ts` now asserts the manifest's event-to-script mapping to prevent regression. The plugin version was bumped to 0.2.0 in the same commit to mark the first usable plugin shipment.
- `mcp-servers/quack.json` and `commands/quack-install.md` are auto-discovered without a manifest (file presence is enough). Only `hooks/` needs the explicit registration file.
- AC.11 smoke (2026-05-13). Ran against Gemini's OpenAI-compatibility endpoint (`https://generativelanguage.googleapis.com/v1beta/openai/`) with `QUACK_MODEL_NAME=gemini-3-flash-preview` — full pipeline green: `docker compose up` → `/health` ok → `bun run build:hook` → `claude plugin marketplace add /Users/ns/workspace/quack` + `claude plugin install quack@quack` (installs at `~/.claude/plugins/cache/quack/quack/0.2.0/`) → per-workspace `(user, project) = (demo, demo)` token minted via admin MCP `create_project` / `register_user` / `add_member` (proxy for the `/quack:install demo` slash command body) → `.envrc` written with `chmod 600` → `claude -p` session from the workspace produced 6 envelopes accepted, 0 extraction errors, 6 Decision + 9 Entity + 6 File + 1 Feedback nodes in Neo4j (`project_id=2`); `search_memory({entities:["Duckling","mascot","rubber duck"]})` recalled both expected entities with the canonical `<memory>…</memory>` wrap. **Note:** AC.5 verification path was the admin-MCP-tool flow rather than the slash command markdown body interpreted by a child Claude session — the underlying behavior matches the spec but a future smoke could swap in `claude -p "/quack:install demo"` to exercise the prompt itself.
