...
- AC-BKPM28.3: `.env.example` includes every env var read by `src/shared/env.ts` (`PORT`, `QUACK_BOOTSTRAP_TOKEN`, `QUACK_DATA_DIR`) with an inline comment explaining each. The file documents that `QUACK_BOOTSTRAP_TOKEN` is consumed only on first boot and must be set before the first `docker compose up`. `.env` (without `.example`) is in `.gitignore` (already covered by M1).
