...
- **Memory-plane MCP surface (M3+):** Retrieval is exposed as **four primitive MCP tools** (read-side), all member-or-admin readable, all returning **structured DTOs only — never prose**: `search_memory`, `get_neighbors`, `path_between`, `recent_decisions`. Every response carries a mandatory `meta` envelope (`{ mode_used, coverage, warnings, explain? }`) so the caller can detect weak retrievals. The query-planning role lives with the caller (Claude Code); Quack is the dumb-server end of the split. Results are wrapped in `<memory>…</memory>` at the MCP boundary and surfaced as untrusted text. **M5 adds one write tool, `add_memory({ content })`** — fire-and-forget; wraps content in a synthetic `kind: "explicit_add"` hook envelope and routes through the same FR-4NY6S1 extractor + redaction + MERGE pipeline that hooks use. No status polling — verify via `search_memory`. Never write to the graph by any path other than the extractor pipeline (LLM-digest-first invariant). **M9 adds one read tool, `ask_memory({ question })`** (FR-WB3N9H) — the **sole exception** to the prose-free / dumb-server split: a bounded server-side agentic loop (cheap model over `QUACK_MODEL_*`) that plans retrieval by calling the four primitives internally and returns a synthesized natural-language `answer`. The four primitives stay prose-free; `ask_memory` carries defense-in-depth (only the `project_id`-scoped read primitives are exposed — no NL→Cypher; retrieved content redacted before re-prompting; untrusted-data framing; `answer` is `<memory>`-wrapped). Its `meta.mode_used` is `"planned"`, realizing the enum reserved in AC-DPY5GQ.8.
...
- **Delivery model (M4+):** Quack ships as **two complementary artifacts in one repo**: (a) a self-hosted server (Docker Compose stack at the repo root) — the source of truth; (b) a Claude Code marketplace plugin (`plugins/quack/`) for client-side hooks + setup command. The plugin is a packaging convenience that wraps the hook logic in thin `bunx` shell scripts. Per-workspace configuration lives in a single project-scoped `.mcp.json` at the workspace root, written by `/quack:install`: it declares the Quack MCP server (literal non-admin single-project token + server URL) and carries an `X-Quack-Sub-Project` header. Claude Code reads `.mcp.json` natively; the hooks read the same file for server URL + token + sub-project — one config artifact, no env vars. **Self-hosted only — no SaaS instance.** When a user installs the plugin from the marketplace, only `plugins/quack/` is downloaded; server code, Compose stack, specs, and other repo internals stay out of the install path.
...
- Server-side natural-language answer synthesis — out of scope for the four primitives (they return structured DTOs only; Claude Code synthesizes). **Exception (M9, FR-WB3N9H):** the single `ask_memory` tool performs server-side synthesis behind defense-in-depth (only the existing `project_id`-scoped read primitives are used — no NL→Cypher; retrieved content redacted; untrusted-data framing; `<memory>`-wrapped answer). No other MCP tool returns prose.
- Dynamic Cypher / NL→Cypher in the read path — security milestone; reserved until per-project isolation is re-architected. (Unchanged by FR-WB3N9H: `ask_memory` generates no Cypher — it calls only bounded server-owned templates, so tenancy isolation is untouched.)
...
| AC-A9BN0M.1 (`HookEnvelope.sub_project?` optional slug-validated field) | `plugins/quack/hooks/_lib/shared/envelope.ts`, `src/ingest/handler.ts` | `plugins/quack/hooks/_lib/__tests__/`, `src/ingest/handler.test.ts` |
| AC-A9BN0M.2 (`source: list<string>` property on all five memory-plane node labels) | `specs/technical-spec.md` §2 graph schema, `src/graph/templates/extract/*.ts` | `src/graph/templates/extract/*.test.ts` |
| AC-A9BN0M.3 (`$source` set-union param on five node-upsert MERGE templates) | `src/graph/templates/extract/upsert_{entity,decision,file,symbol,feedback}.ts` | `src/graph/templates/extract/*.test.ts` |
| AC-A9BN0M.4 (writer threads `sub_project` → `$source`, distinct from project_id override) | `src/extract/writer.ts` | `src/extract/writer.test.ts` |
| AC-A9BN0M.5 (four read-side tools gain optional `sub_projects?: string[]` filter) | `src/mcp/tools/memory/{search_memory,get_neighbors,path_between,recent_decisions}.ts` | per-tool tests under `src/mcp/tools/memory/` |
| AC-A9BN0M.6 (`sub_projects` predicate parameterized in Cypher memory templates) | `src/graph/templates/memory/*.ts` | per-tool tests + `src/graph/templates/memory/*.test.ts` |
...
| AC-ATBKZV.1–4 (typed MCP tool inputSchema; array args drivable; `invalid_args` contract preserved; uniform shared `reg`) | `src/mcp/server.ts` | `src/mcp/server.test.ts` |
